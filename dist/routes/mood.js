...
router.post('/', auth_1.userMiddleware, async (req, res) => {
    try {
        const { moodScore, categories, responses, questions, recommendations } = req.body;
...
router.get('/history', auth_1.userMiddleware, async (req, res) => {
    try {
        const userId = req.user.id;
...
router.get('/latest', auth_1.userMiddleware, async (req, res) => {
    try {
        const userId = req.user.id;
...
router.get('/stats/average', auth_1.userMiddleware, async (req, res) => {
    try {
        const userId = req.user.id;
        const now = new Date();
        // Get weekly average
        ...
        // Calculate averages
        const calculateAverages = (moodArray) => {
