 ...
 * @access Private
 */
router.get('/:id', auth_1.userMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            res.status(401).json({ error: 'Unauthorized' });
 ...
 * @access Private
 */
router.post('/', auth_1.userMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            res.status(401).json({ error: 'Unauthorized' });
        ...
        const { date = new Date().toISOString(), finalScore, quality, duration, consistency, environment, habits, dailyRecommendations } = req.body;
 ...
 * @access Private
 */
router.put('/:id', auth_1.userMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            res.status(401).json({ error: 'Unauthorized' });
        ...
        const { id } = req.params;
        const { finalScore, quality, duration, consistency, environment, habits, dailyRecommendations } = req.body;
 ...
 * @access Private
 */
router.delete('/:id', auth_1.userMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            res.status(401).json({ error: 'Unauthorized' });
