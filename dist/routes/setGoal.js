...
router.post('/health-goal', auth_1.userMiddleware, async (req, res) => {
    const schema = zod_1.z.object({
        goal: zod_1.z.enum(['Lose weight', 'Improve sleep', 'Gain muscle', 'Manage stress'], {
            required_error: "Please select a health goal.",
        }),
        age: zod_1.z.number().min(1, "Age is required"),
        weight: zod_1.z.number().min(1, "Weight is required"),
...
router.get('/health-goal', auth_1.userMiddleware, async (req, res) => {
    try {
        //@ts-expect-error: no need here
        const userId = req.user.id;
...
router.post('/daily-log', auth_1.userMiddleware, async (req, res) => {
    const schema = zod_1.z.object({
        waterIntake: zod_1.z.number().min(0, "Water intake must be a positive number"),
        mood: zod_1.z.string().min(1, "Mood is required"),
        weight: zod_1.z.number().min(0, "Weight must be a positive number"),
        sleepHours: zod_1.z.number().min(0, "Sleep hours must be a positive number"),
        steps: zod_1.z.number().int().min(0, "Steps must be a positive integer"),
        mealQuality: zod_1.z.string().min(1, "Meal quality is required"),
        symptoms: zod_1.z.string().optional(),
...
router.get('/daily-logs', auth_1.userMiddleware, async (req, res) => {
    try {
        //@ts-expect-error: no need here
        const userId = req.user.id;
...
router.get('/daily-log/latest', auth_1.userMiddleware, async (req, res) => {
    try {
        //@ts-expect-error: no need here
        const userId = req.user.id;
